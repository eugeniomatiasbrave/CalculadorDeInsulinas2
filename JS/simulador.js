...
const modal = new bootstrap.Modal(document.getElementById('detalles-modal'));
const botonEliminar = document.getElementById('modal-botonEliminar');



class AltaAfiliado { ///------constructor de Afiliados
   static id= 1;
...
const eventoGuardar = (e) => {   //-----------------------para dar de alta a los Afiliados-----------------------//--Evento 1  // 
    e.preventDefault();
...
const mostrarAltas=()=> {  //-----mostrar altas con foreach
  let Altas = document.getElementById('Altas');  //----------------------------------- DOM de altas

  let AltasLista='';
    AltaAfiliados.forEach ( AltaAfiliado => {
      AltasLista += `
      <div class="card my-2 ms-2 " style="width: 43rem; height: 45px">
     <div class=" row justify-content-between pt-2 ms-3" style="width: 42rem;">
          <table class="table-hover col-9">
          <tr>
          ...
          <div class="col-3 ">
          <input onclick="detalles(${AltaAfiliado.id})" class="bo2 " value="Eliminar"></input>     
          </div>
          </div>
      </div>`    
      })  
   ...
   const Ocultar=()=> {
       document.getElementById('Altas').style.display="none";  // funcion ocultar altas con onclick
   }

   const Mostrar=()=> {
       document.getElementById('Altas').style.display="block"; // funcion mostrar altas con onclick
   }
...
const detalles=(id)=>{
    const AltaAfiliado=AltaAfiliados.find(el=>el.id===id);
       if(!AltaAfiliado){
         return;}
         modalTitle.innerText=`Usted esta por eliminar al Paciente!!`
         modalBody.innerHTML=`<h5>¿Confirma eliminar al paciente ${AltaAfiliado.nombreyapellido}?</h5>`
         modal.show();
         botonEliminar.setAttribute('ref',id);
         }

const eliminarAlta=(e)=>{
...
botonEliminar.addEventListener('click',  eliminarAlta);
...
let contenedorBusquedaAfi = document.getElementById("contenedorBusquedaAfi") //BUSCADOR DE AFILIADOS/////----------DOM de registro
       ...
       <div class="card-header" id="color4"><h6>BUSQUEDA DE PACIENTES</h6></div>
        <form class="container-fluid text-center py-lg-2" id=""  > 
         <div class=" d-flex my-1">
          <input type="number" id="inputBusquedaAfi" class="form-control me-2"  placeholder="Ingresar Nº de Credencial" required>
  ...
  contenedorBusquedaAfi.addEventListener('submit', busquedaAfi); //----------------------------------evento buscar Afiliado
    function busquedaAfi (e){
      ...
      const bu =AltaAfiliados.find(Altas=> Altas.credencial === inputBusquedaAfi); //--------------------Metodo buscar Afi. por credencial
      
      if( !bu){
        Swal.fire({
          title: 'Credencial No encotrada!!',
